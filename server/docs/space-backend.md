...
> The API does not run physics. An
...
is missing or unavailable it uses the bundled default skin and shows a
...
materials. Client terrain, hosted collision and server v7 voxel LOD all use the
...
| `player_snapshots` | Immediate/periodic/disconnect | Latest runtime position and yaw; no backpack data |
...
definitions/snapshots are revision-checked here; legacy browser copies are not used.
